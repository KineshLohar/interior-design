...
export const ServicesSection = () => {
    return (
        <section className="relative w-full flex flex-col items-center text-center bg-gradient-to-tr from-[#F4ECF7] via-[#E4D4C8] to-[#F4ECF7] text-black min-h-dvh p-4 py-10 sm:p-8 pb-24 sm:pt-16 sm:pb-24 md:p-12 md:py-24 lg:p-24">
                ...
                <Separator className="w-full min-h-0.5 rounded-full bg-neutral-700" />
                {
                    services?.slice(0, 4)?.map(serv => (
                        <Fragment key={serv?.id}>
                            <div className="flex flex-col gap-4 md:gap-8 items-stretch md:flex-row md:items-center md:justify-between px-4">
                                <div className="w-full md:w-6/12 space-y-2">
                                    <h3 className="text-xl md:text-2xl font-semibold tracking-wide">{serv?.title}</h3>
                                    <p className="font-lato text-xs sm:text-sm md:text-base text-neutral-800 font-medium">{serv?.description}</p>
                                </div>
            ...
            <div className="mt-8">
                <Link href='/services'>
                    <Button
                        type="button"
                        className=" w-36 py-5 rounded-md tracking-wider cursor-pointer border-[2px] border-neutral-900 bg-neutral-900 font-semibold"
                    >
                        Explore <ArrowRight />
                    </Button>
                </Link>
